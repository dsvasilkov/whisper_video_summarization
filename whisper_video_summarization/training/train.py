...
def train(cfg, dataset_path: Optional[str] = None):
    ...
    if dataset_path:
        ...
        raise ValueError("dataset_path is required. Please upload a dataset through Streamlit or provide a path.")

    train_dataset = GazetaDataset(
        str(train_path), tokenizer, cfg.data.max_length
    )
    val_dataset = GazetaDataset(
        str(val_path), tokenizer, cfg.data.max_length
    )
